...
const tests = {
  ...
  async performance(token) {
    ...
    assert(duration < 3000, '并发请求', '3个并发请求应该在3秒内完成');
    ...
    assert(avgTime < 1000, '响应时间', '平均响应时间应该小于1000ms（JWT缓存生效）');
...
if (require.main === module) {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    ...
    console.log('\n测试覆盖:');
    console.log('  AUTH (4):     注册、登录、获取用户信息、刷新 Token');
    console.log('  CHILDREN (4):  创建、列表、更新、删除');
    console.log('  PHOTOS (5):   列表、上传URL、获取URL、删除、验证');
    console.log('  MEMBERS (3):  获取成员、创建邀请、获取邀请');
    console.log('  PERF (2):     并发请求、响应时间');
    process.exit(0);
  }
